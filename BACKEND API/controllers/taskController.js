...
exports.createTask = async (request, response) => {
	try {
		...
		if (task) {
			return response
				.status(400)
				.json({ success: false, responseMessage: `task already exists with title ${task.title}` });
		}
		// create new task
...
exports.getTaskById = async (request, response) => {
	try {
		...
		if (!Mongoose.isValidObjectId(task_id)) {
			return response.status(400).json({ success: false, responseMessage: `invalid id ${task_id}` });
		}
...
exports.updateTask = async (request, response) => {
	try {
		...
		if (!task) {
			return response
				.status(404)
				.json({ success: false, responseMessage: `task not found with id ${task_id}` });
		}
		// update task
